  ...
  export default {
    ...
    methods: {
      handleUpload() {
        ...
        this.getFileMd5(currentFile.raw, async (md5) => {
          ...
          if (checkResult.status === 1) {
            ...
            return
          } else if (checkResult.status === 2) {  // "上传中" 状态
            // 获取已上传分片列表
            console.log(checkResult)
          ...
          // 4. 获取上传url
          let uploadUrls;
          if(checkResult!=null&& checkResult.uploadId!=null){
            console.log(checkResult);
            self.uploadIdInfo=checkResult.uploadId
            self.saveFileUploadId(checkResult.uploadId)
            uploadUrls= checkResult.uploadUrls
          }else{
          let uploadIdInfoResult = await self.getFileUploadUrls(param)
          console.log(uploadIdInfoResult.data)
          self.uploadIdInfo = uploadIdInfoResult.data.uploadId
          self.saveFileUploadId(uploadIdInfoResult.data.uploadId)
          uploadUrls = uploadIdInfoResult.data.uploadUrls
          }
          console.log(fileChunks.length)
          console.log(uploadUrls.length)
      ...
      getFileUploadUrls(fileParam) {
        let url = `http://localhost:8838/multipart/init?_=${Math.random()}`
      ...
      checkFileUploadedByMd5(md5) {
        let url = `http://localhost:8838/multipart/check?md5=${md5}`
        return new Promise((resolve, reject) => {
      ...
      mergeFile(file) {
        const self = this
        let url = `http://localhost:8838/multipart/complete?uploadId=${file.uploadId}&fileName=${file.fileName}&md5=${file.md5}`
        return new Promise((resolve, reject) => {
